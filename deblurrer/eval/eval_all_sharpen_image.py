...
for step in range(12, 20):
    ...
    report_name = "train_without_postprocessing_no_upsampling_report_step_" + str(step) + "_model_" + identifier
    report_path = path_parts[:-1]
    report_path.append(report_name)
    report_path = os.path.join(*report_path)
    #Path(report_path).mkdir(parents=True, exist_ok=True)
    
    for i, batch in tqdm(zip(range(3), dataset.train_dataloader()),
                         total=3):
        with torch.no_grad():
            gt, obs, _ = batch
            ...
            filter_reco = ndimage.gaussian_filter(reco, 4)

            alpha = 100/255.
            sharpened = reco + alpha * (reco - filter_reco)

            fig, (ax1, ax2) = plt.subplots(1,2)
            ax1.imshow(reco, cmap="gray")
