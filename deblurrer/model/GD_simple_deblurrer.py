...
class IterativeReconstructor(pl.LightningModule):
    def __init__(self, lr=1e-4, downsampling_steps=3, n_iter=8,
                 batch_norm=True,channels=[2,4, 8, 16, 16], skip_channels=[2,4,8,16,16], radius=5.5, img_shape=(1460, 2360), kappa=0.03, 
                 regularization='pm', use_sigmoid=True, jittering_std=0.0, loss="l1", op_init=None, kappa_wiener=0.001):
        super().__init__()
        # img_shape: 181, 294
        ...
        self.net = IterativeDeblurringNet(n_iter=self.n_iter, op=self.blur, op_reg=op_reg, op_init=op_init, use_sigmoid = self.use_sigmoid,
            channels=self.channels, skip_channels=self.skip_channels,
...
class IterativeDeblurringNet(nn.Module):
    def __init__(self, n_iter, op, op_reg, op_init=None,
        ...
        if self.op_reg is not None:
            ...
            for it in range(n_iter):
                self.iterative_blocks.append(IterativeDeblurringBlock(
                    n_in=3, n_out=1, n_memory=0, batch_norm=batch_norm,channels=channels, skip_channels=skip_channels))
        else: 
            self.iterative_blocks = nn.ModuleList()
            for it in range(n_iter):
                self.iterative_blocks.append(IterativeDeblurringBlock(
                    n_in=2, n_out=1, n_memory=0, batch_norm=batch_norm,channels=channels, skip_channels=skip_channels))
    ...
    def forward(self, y, it=-1):
        ...
        for i in range(n_iter):
            grad = self.op.filtered_grad(x_cur[:, 0:1, ...], y)
            x_cur = x_cur - 0.001*grad

            if self.op_reg is not None: 
                pm = self.op_reg(x_cur[:, 0:1, ...])
                x_update = torch.cat([x_cur, pm, y], dim=1)

            else:
                x_update = torch.cat([x_cur, y], dim=1)
